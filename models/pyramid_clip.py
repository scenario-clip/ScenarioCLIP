...
class PyramidCLIP(L.LightningModule):
    ...
    def configure_optimizers(self, warmup_ratio: float = 0.10, weight_decay: float = 0.2, lr_floor: float = 0.0):
        if self.mode == "linear-probing":
            ...
            opt = AdamW(trainable, lr=self.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0)
            return opt
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mode == "pretrain":
            ...
            self.log_dict({
                "train_loss_total": contrastive_loss,
                "train_loss_global": global_contrastive_loss,
                "train_loss_object": object_contrastive_loss,
                "train_loss_relation": relation_contrastive_loss,
                "logit_scale_global":   self.global_loss.logit_scale.exp().clamp(max=self.global_loss.max_scale),
                "logit_scale_object":   self.object_loss.logit_scale.exp().clamp(max=self.object_loss.max_scale),
                "logit_scale_relation": self.relation_loss.logit_scale.exp().clamp(max=self.relation_loss.max_scale),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.mode == "pretrain":
            ...
            self.log_dict({
                ...
                "val_contrastive_global": global_contrastive_loss,
                "val_contrastive_object": object_contrastive_loss,
                "val_contrastive_relation": relation_contrastive_loss,
            ...
            if self.action_test:
                ...
                if len(os.listdir(action_dir)) > 0:
                    ...
                    for i, action in enumerate(self.actions):
                        ...
                        action_embedding = self.model.encode_text(torch.unsqueeze(caption_tokens.input_ids, 0))
            ...
            if self.object_test:
                ...
                if len(os.listdir(object_dir)) > 0:
                    ...
                    for i, object in enumerate(self.objects):
                        ...
                        object_embedding = self.model.encode_text(torch.unsqueeze(caption_tokens.input_ids, 0))
            ...
            if self.relation_test:
                ...
                if len(os.listdir(relation_dir)) > 0:
                    ...
                    for i, relation in enumerate(self.relations):
                        ...
                        relation_embedding = self.model.encode_text(torch.unsqueeze(caption_tokens.input_ids, 0))
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.mode == "zero-shot":
            ...
            if self.object_test:
                object_images = torch.stack([img for obj_list in objects_cropped for img in obj_list]).to(self.device)
                object_embeddings = self.model.encode_image(object_images)
            ...
            if self.relation_test:
                ...
                relation_embeddings = self.model.encode_image(relation_images_flat)
